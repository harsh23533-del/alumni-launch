...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(data.role === 'alumni' ? '/alumni/dashboard' : '/startups');
    } catch (err) {
  ...
  return (
    <div className="page" style={{ maxWidth: 420, paddingTop: 60 }}>
      ...
      <p style={{ marginTop: 18, fontSize: 14, color: 'var(--text-dim)', textAlign: 'center' }}>
        New here? <Link to="/signup/alumni" style={{ color: 'var(--ink)', fontWeight: 600 }}>Alumni sign up</Link>
        {' · '}
        <Link to="/signup/student" style={{ color: 'var(--ink)', fontWeight: 600 }}>Student sign up</Link>
      </p>
    </div>
  );
}
